	...
	names = null;
		...
		directory = {
			...
			map: {
				...
				id: 'occupydirectory.map-ftbke77j',
		...
		directory.initMap = function( el ){
			...
			console.log( ":::::", directory.map.id );

//			directory.map.instance = mapbox.map( 'map', mapbox.layer().id( directory.map.id ) ).zoom( 3 );

			directory.map.instance = mapbox.load( directory.map.id, function( o ){
				var map = mapbox.map('map');
				console.log( "::", o, o.layer, o.data[0] );
				map.addLayer( o.layer ).zoom( directory.map.zoom ).center( directory.map.center );
//				map.addLayer( o.data[0] );
...
//							obj.data.__full__ = true;
							console.log( 'click', arguments );//obj, sidebarContent );
