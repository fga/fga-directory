...
directory.share = function(map, tilejson) {
    ...
    embed.value = '<div id="embed-wrapper" style="width:960px; height:700px"><iframe id="directory-embed" width="960" height="600" src="{{embed}}" style="border:0; height:100%; width:100%; position:absolute;" marginwidth="0" marginheight="0" frameborder="0" vspace="0" hspace="0" allowtransparency="true" scrolling="no" ></iframe><script type="text/javascript">document.getElementById("directory-embed").src = "{{embed}}" + "phostname=" + window.location.hostname;</script></div>'.replace('{{embed}}', tilejson.embed + "&phostname=" + window.location.hostname );
...
directory.initMap = function( el ){
			...
			mapbox.load( directory.map.id, function( o ){
                ...
                if( $.inArray('share', directory.map.options ) > -1 ){
                    directory.share( map,  map.layers[0].tilejson() ).appendTo( $( 'body' )[0] );
                    directory.map.options.splice($.inArray( "share", directory.map.options),1);
                }
...
directory.geocoder = function() {
    $('[data-control="geocode"] form').submit(function(e) {
        e.preventDefault();
        geocode($('input[type=text]', this).val(), directory.map.map );
    });
