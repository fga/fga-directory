        ...
        wax.tilejson( l.api, function(t) {
            ...
            MM_map = new MM.Map( 
                ...
                lon: (l.center) ? l.center.lon : t.center[0]
						...
						Map.ea = easey().map( MM_map );//.easing('easeInOut');
            ...
            for (var i = 0; i < l.features.length; i++) {
                switch(l.features[i]) {
                    ...
                    case 'bwdetect':
                        ...
                        break;
                    case 'share':
												if( wax.mm, wax.mm.share ) wax.mm.share( MM_map, t ).appendTo( $('body')[0] );
                        break;
                    case 'tooltips':
                        MM_map.interaction = wax.mm.interaction()
												...
												.on({
													on: function( o ){
														wax.tooltip().animate(true).parent($('body')[0] ).events();
														console.log( wax.tooltip() );
													}
												});
                        MM_map.interaction.on( {
													on: function( o ){
														console.log( 'on', o );
													},
													off: function( o ){
...
function bindGeocoder() {
    ...
    var geocode = function(query, m) {
			...
			reqwest({
				...
				success: function (r) {
					r = r[0];
					if (MM_map.geocodeLayer) MM_map.geocodeLayer.removeAllMarkers();
					$('form.geocode').removeClass('loading');
						if (r === undefined) {
							$('#geocode-error').text('This address cannot be found.').fadeIn(650);
						} else {
							$('#geocode-error').hide();
							MM_map.setExtent([
							...
							if (MM_map.getZoom() === MM_map.coordLimits[1].zoom) {
								...
								MM_map.geocodeLayer = mmg().geojson(point);
								MM_map.addLayer(MM_map.geocodeLayer);
								}
							}
							console.log( r.lat, r.lon );
							easey().map( MM_map )
								.to( MM_map.locationCoordinate( { lat: r.lat, lon: r.lon } ) )
								.path('about')
								.run( 4000 , function(){ 
									console.log( 'ease ended' );
								});
//										MM_map.setCenter({ lat: r.lat, lon: r.lon });
        ...
        // if ( $('.wax-attribution') && $('.wax-attribution').html().indexOf( attribution ) < 0) {
